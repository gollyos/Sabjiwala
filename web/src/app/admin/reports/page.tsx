      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-xs">
          ...
          <p className="text-xs text-slate-500 mt-1 max-w-2xl">
            Select a specialized report category below to view live metrics, profit margins, sales waterfalls, and export spreadsheets.
          </p>
        </div>
          ...
          {reportCategories.map((cat, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={cat.href}
                className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs hover:shadow-md hover:border-emerald-400 transition-all duration-200 flex flex-col justify-between group cursor-pointer"
              >
                <div className="space-y-3">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-900 text-base tracking-tight group-hover:text-emerald-700 transition-colors">
                      {cat.title}
                    </h3>
