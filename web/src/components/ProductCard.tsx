...
import { Product } from '@/types/taji-tokri';
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className={`group bg-white dark:bg-[#0b1b14] rounded-2xl sm:rounded-3xl overflow-hidden border shadow-[0_8px_30px_rgb(23_59_40/0.06)] transition-[box-shadow,border-color,transform] duration-300 hover:-translate-y-0.5 flex flex-col justify-between h-full min-w-0 relative isolate ${
      !isAvailable
        ? 'border-slate-200 dark:border-slate-800 opacity-75'
        : 'border-slate-200 dark:border-slate-800 hover:shadow-[0_16px_36px_rgb(238_138_47/0.16)] hover:border-[#f6a94a]/70 dark:hover:border-[#f6a94a]/40'
    }`}>
      ...
      <div>
        <div className="relative w-full aspect-4/3 bg-slate-50 dark:bg-slate-950 overflow-hidden">
          ...
          <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10">
            {isAvailable ? (
              <>
                ...
                {product.is_seasonal && (
                  <span className="px-2 py-0.5 rounded-full bg-[linear-gradient(135deg,#ee8a2f_0%,#e0453a_100%)] text-white text-[9px] font-bold shadow-2xs">
                    Seasonal
                  </span>
                )}
              </>
          ...
          <div className="font-display absolute bottom-2.5 right-2.5 px-2.5 py-1 rounded-xl bg-[linear-gradient(135deg,#0f7a45_0%,#0a5c35_100%)] text-white font-extrabold text-sm tabular-nums shadow-md z-10">
        ...
        <div className="p-3 sm:p-4">
          <h3 lang="gu" className="font-display font-bold text-sm sm:text-base text-slate-900 dark:text-white leading-tight truncate">
            {product.name_gu}
          </h3>
      ...
      <div className="p-3 sm:p-4 pt-0">
        {!isAvailable ? (
          ...
          <button
            onClick={handleAdd}
            className="w-full min-h-11 py-2.5 sm:py-3 px-3 sm:px-4 bg-[linear-gradient(135deg,#0f7a45_0%,#0a5c35_100%)] hover:brightness-110 active:scale-[0.98] text-white text-sm font-extrabold rounded-2xl shadow-md shadow-emerald-900/15 flex items-center justify-center space-x-1.5 transition-[filter,transform] cursor-pointer"
          >
