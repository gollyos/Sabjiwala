...
export function OrderSuccessModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-emerald-100 animate-scale-up text-center p-6 sm:p-8">
        ...
        <div className="w-20 h-20 rounded-3xl bg-emerald-100 text-emerald-600 flex items-center justify-center mx-auto mb-4 shadow-inner ring-8 ring-emerald-50">
        ...
        {/* Order Details Card */}
        <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200/80 text-left space-y-3 mb-6">
          <div className="flex justify-between items-center pb-2.5 border-b border-emerald-200/60">
            <span className="text-xs font-semibold text-slate-600">Order Number</span>
          ...
          {orderSuccessData.delivery_date && (
            <div className="flex items-center space-x-2.5 text-xs text-slate-700">
              ...
              <span>
                Delivery Date: <strong>{new Date(orderSuccessData.delivery_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</strong>
              </span>
            </div>
          )}
          ...
          {orderSuccessData.first_order_discount > 0 && (
            <div className="p-2 rounded-xl bg-emerald-100/80 text-[11px] font-semibold text-emerald-900 flex items-center gap-1.5">
              <Tag className="w-3.5 h-3.5 text-emerald-700" />
              <span>You saved ₹{orderSuccessData.first_order_discount.toFixed(2)} with FIRST500 offer!</span>
            </div>
          )}

          <div className="flex justify-between items-center pt-2.5 border-t border-emerald-200/60 text-xs">
        ...
        <div className="space-y-2">
          <Link
            href="/profile"
            onClick={closeOrderSuccessModal}
            className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-lg shadow-emerald-600/20 flex items-center justify-center space-x-2 transition-all cursor-pointer text-sm"
          >
