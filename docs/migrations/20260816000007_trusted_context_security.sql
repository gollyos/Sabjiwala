-- =============================================================================
-- TRUSTED CONTEXT FOR PROTECTED SECURITY FIELDS TRIGGER
-- =============================================================================
...
DECLARE
    is_staff BOOLEAN;
    is_trusted_internal BOOLEAN;
BEGIN
    -- Check if session flag is set by trusted RPCs
    IF current_setting('sabjiwala.trusted_context', true) = 'true' THEN
        RETURN NEW;
    END IF;
...
DECLARE
    ...
    v_seq INT;
BEGIN
    -- Enable trusted context for this transaction
    PERFORM set_config('sabjiwala.trusted_context', 'true', true);
